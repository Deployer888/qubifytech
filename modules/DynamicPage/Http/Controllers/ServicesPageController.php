...
class ServicesPageController extends Controller
{
    public function index()
    {
        ...
        $pageType = $this->getOrCreatePageType($page->id, 'services');
    ...
    public function saveServiceCardSection(Request $request, $sectionName): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'features' => 'required|string|max:500',
            'button_text' => 'required|string|max:255',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048', // New validation for image file
            'image_alt' => 'nullable|string|max:255', // New validation for image alt text
            'is_active' => 'boolean'
        ]);
        ...
        try {
            ...
            $existingContent = DynamicContent::where([
                'page_id' => $page->id,
                'page_type_id' => $pageType->id,
                'section_name' => $sectionName
            ])->first();
            ...
            if ($request->hasFile('image')) {
                ...
                if ($existingContent && isset($existingContent->content_json['image_url'])) {
                    $oldImagePath = str_replace('storage/', '', $existingContent->content_json['image_url']);
            ...
            } elseif ($existingContent && isset($existingContent->content_json['image_url'])) {
                // Keep existing image if no new image is uploaded
                $imageUrl = $existingContent->content_json['image_url'];
            }
            ...
            $contentData = [
                ...
                'image_url' => $imageUrl,
                'image_alt' => $request->image_alt // Add image alt text to content
            ];
